...
public class MainActivity extends AppCompatActivity {
    ...
    private ArrayList<Byte> inputAudio = new ArrayList<>();
    ...
    private void startTranscribe(ArrayList<Byte> inputAudio, int sampleRate, int channels, int audioFormat) {
        try {
            Log.e(TAG, fileName);
            transcribeResultTxt.setText(loadModelJNI(getAssets(), inputAudio, sampleRate, channels, audioFormat, 1));
    ...
    private native String loadModelJNI(AssetManager assetManager, byte[] buffer, int sampleRate, int channels, int audioFormat, int is_recorded); //private native String loadModelJNI(AssetManager assetManager, String fileName, int is_recorded);
    ...
    public class AudioListener implements BufferFilledListener {
        @Override
        public void onBufferFilled(byte[] buffer) {
    ...
    public static class WavAudioRecorder {
        ...
        // Listener when the buffer reads audio
        private BufferFilledListener bufferFilledListener;
        ...
        private AudioRecord.OnRecordPositionUpdateListener updateListener = new AudioRecord.OnRecordPositionUpdateListener() {
            ...
            public void onPeriodicNotification(AudioRecord recorder) {
                ...
                try {
                    ...
                    // called buffer listener
                    if (bufferFilledListener != null) {
                        bufferFilledListener.onBufferFilled(buffer);
                    }
                } catch (IOException e) {
        ...
        /**
         * Sets an buffer listener, call after construction/reset and before prepare
         *
         * @param listener listener that gets called when audio is read
         */
        public void setBufferFilledListener(BufferFilledListener listener) {
            try {
                if (state == State.INITIALIZING) {
                    this.bufferFilledListener = listener;
                }
            } catch (Exception e) {
                if (e.getMessage() != null) {
                    Log.e(WavAudioRecorder.class.getName(), e.getMessage());
                } else {
                    Log.e(WavAudioRecorder.class.getName(), "Unknown error occured while setting the audio listener");
                }
                state = State.ERROR;
            }
        ...
        public void reset() {
            try {
                if (state != State.ERROR) {
                    ...
                    bufferFilledListener = null; // Reset buffer listener
