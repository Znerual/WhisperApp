...
#define INFERENCE_ON_AUDIO_FILE 0 // 1
...
Java_com_whisper_android_tflitecpp_MainActivity_loadModelJNI(
        ...
        jbyteArray inputBuffer, // new
        jint sampleRate, // new
        jint channels, // new
        jint audioFormat, // new
        //jint inputBufferSize, // new
        // jstring fileName,
        jint is_recorded) {
    ...
    if (INFERENCE_ON_AUDIO_FILE) {
        ...
        // convert byte audio buffer to float32
        jsize inputLength = env->GetArrayLength(inputBuffer);
        jbyte* inputData = env->GetByteArrayElements(inputBuffer, NULL);
        std::vector<float> pcmf32;
        if (channels == 1) {
            for (int i = 0; i < inputLength / 2; i++) {
                int16_t sample = (inputData[i * 2] & 0xFF) | (inputData[i * 2 + 1] << 8);
                pcmf32.push_back(float(sample) / 32768.0f) ;
            }
        } else {
            for (int i = 0; i < inputLength / 4; i++) {
                int16_t sample1 = (inputData[i * 2] & 0xFF) | (inputData[i * 2 + 1] << 8);
                int16_t sample2 = (inputData[i * 2 + 2] & 0xFF) | (inputData[i * 2 + 3] << 8);
                pcmf32.push_back(float(sample1 + sample2) / 65536.0f);
            }
        }
    ...
    if(!g_whisper_tflite_params.is_whisper_tflite_initialized) {
        ...
        tflite::InterpreterOptions options = tflite::InterpreterOptions();
